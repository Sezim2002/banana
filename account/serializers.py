...
#         user.save()
#         send_mail('Восстановление пароля', f'Ваш новый пароль: {password}', 'test@test.com', [email])
...
class ForgotPasswordSerializer(serializers.Serializer):
    ...
    def send_code(self):
        ...
        user = User.objects.get(email=email)
        user.create_activation_code()
        send_mail('Восстановление пароля', f'Ваш код подтверждения:{user.activation_code}', 'test@test.com', [email])
