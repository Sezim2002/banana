# представление(обработчик запросов от клиента)
...
class RegistrationView(APIView):
    def post(self, request):
        ...
        if serializer.is_valid():
            serializer.save()
            return Response('Ваш аккаунт успешно зарегистрирован', status=201)
...
# 1. самостоятельно устанавливаем новый пароль
...
# 2. отправляем юзеру код, а он сам создает пароль
class ForgotPasswordView(APIView):
    def post(self, request):
        serializer = ForgotPasswordSerializer(data=request.data)
        if serializer.is_valid():
            return Response('Ва выслан код для восстановления пароля')
        return Response(serializer.errors, status=400)
